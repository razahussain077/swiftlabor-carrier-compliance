...
export default function Home() {
  ...
  async function runReview() {
    ...
    try {
      ...
      setReview({ status:"REVIEW", readiness:62, summary:"Two blocking exceptions and one renewal risk were detected. Resolve the packet before the carrier becomes dispatch-ready.", checks:fallbackChecks.map(([label,value,tone,detail])=>({label,value,tone,detail})), nextStep:"Request updated cargo coverage + signed carrier agreement", ai:false, source:"verified demo fallback" });
      setReviewed(true);
  ...
  return <div className="app">
    ...
    <div className="shell">
      ...
      <main className="main">
        <div className="heroRow"><div><div className="eyebrow">AI OPERATIONS AGENT · 02</div><h1>CarrierGate</h1><p>Turn a new carrier packet into a clear, evidence-backed activation decision.</p></div><div className="agentChip"><div className="agentIcon"><ShieldCheck size={17}/></div><div><strong>Compliance & onboarding</strong><small>AI decision engine · document checks</small></div></div></div>
        ...
        <div className="sectionTitle"><div><div className="eyebrow">02 · DECISION ENGINE</div><h2>Documents → decision</h2></div>{reviewed && <span className="complete"><CheckCircle2 size={14}/> Review complete · {review?.ai ? "AI analyzed" : "Demo fallback"}</span>}</div>
        {!reviewed ? <section className="waiting"><div className="waitingIcon"><ShieldCheck size={23}/></div><strong>Ready to review</strong><p>CarrierGate will analyze the packet against broker requirements, verify public-source evidence, and surface exceptions.</p><div className="pipeline"><span>EXTRACT</span><ChevronRight size={13}/><span>COMPARE</span><ChevronRight size={13}/><span>VERIFY</span><ChevronRight size={13}/><span>DECIDE</span></div></section> : <section className="results">
          <div className="decision"><div><div className="decisionTag"><span>{review?.status}</span><small>AI-RECOMMENDED ACTIVATION STATUS</small></div><h3>Do not activate yet</h3><p>{review?.summary}</p></div><div className="readiness"><span>READINESS</span><strong>{review?.readiness}</strong><small>/ 100</small></div></div>
